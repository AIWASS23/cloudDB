...
//  CloudKitExample
//
//  Created by Marcelo de Araújo on 19/11/23.
//
...
class CloudKitUtility {
    ...
    enum CloudKitError: String, LocalizedError {
        ...
        case saveError
        case deleteError
        case errorInSubscribe
        case errorInUnSubscribe
    }
}
...
extension CloudKitUtility {
    ...
    static func discoverUserIdentity() -> Future<String, Error> {
        .init { result in
            CKContainer.default().fetchUserRecordID { id, error in
                if let id {
                    CKContainer.default().discoverUserIdentity(withUserRecordID: id) { user, error in
...
extension CloudKitUtility {
    ...
    static private func addRecordMatchedBlock<T: CloudKitItemProtocol>(
        for operation: CKQueryOperation,
...
// Posso escolher o tipo do conteiner a ser salvo: public, private e shared

//    static func save<T: CloudKitItemProtocol>(
//            _ item: T,
//            inContainer container: CKContainer = CKContainer.default(),
//            database: CKDatabase = CKContainer.default().publicCloudDatabase
//        ) -> Future<Bool, Error> {
//            .init { result in
//                container.database(with: database.databaseScope).save(item.record) { record, error in
//                    guard error == nil else {
//                        result(.failure(CloudKitError.saveError))
//                        return
//                    }
...
extension CloudKitUtility {
    ...
    static func delete<T: CloudKitItemProtocol>(_ item: T) -> Future<Bool, Error> {
        .init { result in
            CKContainer.default().publicCloudDatabase.delete(withRecordID: item.record.recordID) { id, error in
                guard error == nil else {
...
extension CloudKitUtility {
    ...
    static func requestNotificationPermission() {
        ...
        UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .sound, .badge]) { isEnabled, error in
            guard error == nil else {
    ...
    static func unSubscribeNotifications(_ id: CKSubscription.ID) -> Future<Bool, Error> {
        .init { result in
            CKContainer.default().publicCloudDatabase.delete(withSubscriptionID: id) { _, error in
                guard error == nil else {
                    print("Reason: \(error?.localizedDescription ?? "")")
                    result(.failure(CloudKitError.errorInUnSubscribe))
                    return
